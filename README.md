...
- **固定レイアウト EPUB**（既定）: PDF の 1 ページを EPUB の 1 ページとして収録。ページ画像の上に、座標付きの透明テキスト層を重ねるため、見た目は原本のまま検索・選択・読み上げができる。1 ページ目は表紙（cover-image）として設定され、縦書き（`--vertical`）では右綴じになる
