...
各バージョンの変更点は [CHANGELOG.ja.md](CHANGELOG.ja.md) を参照してください。

## 特徴

![EpubFabric の PDF から EPUB への変換画面](images/screenshot.png)
...
- **レイアウトパターン分類**: ページごとに横書き1段・横書き2段・縦書き1段・縦書き2段を分類し、パターン別の読み順でリフロー。横書き3〜4段や不等幅の複雑紙面は再帰的なガター検出へフォールバック
...
src/
  ...
  EpubFabric.Layout       レイアウト解析: 4パターン分類・段組み（ColumnDetector）・見出し・段落統合
...
3. **レイアウト解析**（リフロー時）: 図版・囲み記事検出、横/縦×1/2段のパターン分類、見出し推定、パターン別読み順、段落統合
