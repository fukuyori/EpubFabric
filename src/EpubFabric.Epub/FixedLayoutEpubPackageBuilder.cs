...
public sealed class FixedLayoutEpubPackageBuilder
{
    ...
    private readonly PageImageTranscoder _transcoder;

    /// <param name="jpegQuality">ページ画像をJPEG化する際の品質（1～100）。</param>
    /// <param name="maxImageSideLength">ページ画像の長辺の上限px。0以下で無制限。</param>
    public FixedLayoutEpubPackageBuilder(
        int jpegQuality = PageImageTranscoder.DefaultJpegQuality,
        int maxImageSideLength = PageImageTranscoder.DefaultMaxImageSideLength)
    {
        _transcoder = new PageImageTranscoder(jpegQuality, maxImageSideLength);
    }
    ...
    public void Build(EpubFabricProject project, string outputEpubPath)
    {
        ...
        foreach (var resource in pageResources)
        {
            ...
            {
                // 再圧縮不要でも収録元は表示用画像（--enhance時は高品質化画像）を使う。
                zip.CreateEntryFromFile(
                    resource.SourceImagePath,
    ...
    {
        var prepared = _transcoder.Prepare(PageImageTranscoder.DisplayImagePathOf(page), $"page-{index + 1:0000}");
        return new PageResource(page, XhtmlFileName(index), prepared.FileName, prepared.Bytes, prepared.SourcePath);
    }

    private static string ImageMediaType(string fileName) => PageImageTranscoder.MediaTypeOf(fileName);
